...
/**
 * @brief Computes the weighted cross-entropy (logistic) loss @f$
 *          E = \frac{-1}{n} \sum\limits_{n=1}^N \left[
 *                  0.5 / w_n * p_n \log \hat{p}_n +
 *                  0.5 / (1 - w_n) * (1 - p_n) \log(1 - \hat{p}_n)
 *              \right]
 *        @f$, often used for predicting targets interpreted as probabilities
          when positive and negative samples are unevenly distributed.
 *
 * This layer is implemented rather than separate
 * SigmoidLayer + WeightedCrossEntropyLayer
 * as its gradient computation is more numerically stable.
 * At test time, this layer can be replaced simply by a SigmoidLayer.
 *
 * The loss display remains the same as the unweighted cross-entropy loss layer.
 *
...
class SigmoidWeightedCrossEntropyLossLayer : public LossLayer<Dtype> {
 ...
 protected:
  ...
  /**
   * @brief Computes the sigmoid weighted cross-entropy loss error gradient w.r.t. the
   *        predictions.
   *
   ...
   *   -# @f$ (N \times 1 \times 1 \times 1) @f$
   *      the weights -- ignored as we can't compute their error gradients
   */
...
#endif  // CAFFE_SIGMOID_WEIGHTED_CROSS_ENTROPY_LOSS_LAYER_HPP_
